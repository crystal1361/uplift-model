...
- `uplift_model_deck.pptx` — a 17-slide walkthrough deck: why uplift over a response model, dedicated slides on S-Learner and T-Learner (how each trains and scores in the credit-card prescreen example, pros/cons, and why not chosen), a mechanistic deep dive on why arm imbalance breaks T-Learner's subtraction, the comparison table and why X-Learner wins, a full numeric walkthrough of the X-learner's four layers (outcome models → pseudo effects → effect models → weighted fusion, each with a plain-language "why this step" explanation), the Qini validation results above, and a one-minute summary (with a bilingual script in the speaker notes of the last slide).
